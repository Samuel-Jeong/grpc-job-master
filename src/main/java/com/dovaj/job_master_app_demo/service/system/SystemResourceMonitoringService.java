package com.dovaj.job_master_app_demo.service.system;

import com.sun.management.OperatingSystemMXBean;
import com.dovaj.job_master_app_demo.config.MonitoringProperties;
import com.dovaj.job_master_app_demo.data.dto.system.SystemMetrics;
...
/**
 * packageName    : com.dovaj.job_master_app_demo.service.system
...
public class SystemResourceMonitoringService {
    ...
    public void sampleAndLog() {
        if (!monitoringProperties.isEnableScheduler()) return;
    ...
    private Severity evaluateSeverity(SystemMetrics systemMetrics) {
        ...
        s = max(s, severityFor(systemMetrics.getCpu().getSystemCpuLoadPercent(),
                monitoringProperties.getCpuWarn(), monitoringProperties.getCpuCrit()));
        ...
        s = max(s, severityFor(systemMetrics.getMemory().getUsedPercent(),
                monitoringProperties.getMemWarn(), monitoringProperties.getMemCrit()));

        // JVM Heap
        s = max(s, severityFor(systemMetrics.getJvm().getHeapUsedPercent(),
                monitoringProperties.getJvmWarn(), monitoringProperties.getJvmCrit()));

        // Disks
        for (SystemMetrics.Disk d : systemMetrics.getDisks()) {
            s = max(s, severityFor(d.getUsedPercent(),
                    monitoringProperties.getDiskWarn(), monitoringProperties.getDiskCrit()));
        }
        return s;
    }
